...
@ApiStatus.Internal
public record CLIOption(String[] path, @Nullable Object sectionInstance, ConfigOption configOption, Object value, Node node) {

    public List<String> getNames() {
        String path = Stream.of(this.path).map(s -> s.replace('.', '-')).collect(Collectors.joining("."));
        List<String> names = new ArrayList<>();
        for (String alias : this.configOption.getCliAliases()) {
            if (alias.isEmpty()) continue;
            names.add(path + alias.replace('.', '-'));
        }
        names.add(path + this.configOption.getCliName().replace('.', '-'));
